...
struct RamdiskRebootService: Sendable {
    func reboot(output: @Sendable @escaping (String) -> Void) async throws {
        try await Task.detached(priority: .userInitiated) {
            try rebootSynchronously(output: output)
        }.value
    }

    private func rebootSynchronously(output: @Sendable @escaping (String) -> Void) throws {
        guard let iproxyURL = AppResourceLocator.toolURL(named: "iproxy") else {
            throw RamdiskRebootError.missingResource("iproxy")
        }

        guard FileManager.default.fileExists(atPath: "/usr/bin/expect") else {
            throw RamdiskRebootError.missingResource("/usr/bin/expect")
        }

        let temporaryRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent(".yx-reboot-\(UUID().uuidString)", isDirectory: true)
        ...
        guard connected else { throw RamdiskRebootError.sshUnavailable }

        output("Ramdisk SSH Connected")
        output("Sending the device reboot command…")
        let command = "if [ -x /sbin/reboot ]; then echo __YX_REBOOT_SENT__; /sbin/reboot -q; elif [ -x /usr/sbin/reboot ]; then echo __YX_REBOOT_SENT__; /usr/sbin/reboot; elif [ -x /bin/launchctl ]; then echo __YX_REBOOT_SENT__; /bin/launchctl reboot userspace; else echo __YX_REBOOT_NOT_FOUND__; exit 127; fi"
        ...
        let hasCommandError = lowerOutput.contains("__yx_reboot_not_found__")
            || lowerOutput.contains("command not found")
            || lowerOutput.contains("permission denied")
            || lowerOutput.contains("no such file")
        let didSendReboot = cleanOutput.contains("__YX_REBOOT_SENT__")
        // SSH readiness has already been verified above. A 255 here means the
        // device terminated the SSH session while rebooting, so it is success.
        let rebootSucceeded = result.status == 255
            || (!hasCommandError && result.status == 0 && didSendReboot)
        guard rebootSucceeded else {
            throw RamdiskRebootError.commandFailed(status: result.status, output: cleanOutput)
        }
        output("Reboot command sent,Device is rebooting")
    }
...
enum RamdiskRebootError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .missingResource(name):
            return "Missing Ramdisk reboot resource: \(name)"
        case .sshUnavailable:
        ...
        case let .commandFailed(status, output):
            ...
            return "Ramdisk reboot command failed (\(status)):\(lastLine)"
        }
    }
}
